...
function lpSeriesSummary(series, { gas_bnb = null } = {}) {
  ...
  if (out.profit) {
    const p = out.profit;
    out.sentence += ` Profit so far ${sign(p.bnb)} BNB${p.usd != null ? ` (about $${p.usd.toFixed(2)})` : ''}: ${sign(p.from_price_bnb)} BNB from the pair's price, ${sign(p.from_fees_bnb)} BNB of fees${p.gas_bnb != null ? `, −${f(p.gas_bnb, 5)} BNB of gas` : ''}.`;
  }
  return out;
}
...
`)}${pageNav({ href: SITE + '/liquidity', label: 'Liquidity' }, { href: '/lp/agent', label: 'The liquidity agent — its record' }, BUY)}<h1>The liquidity agent<span class="st ${last.ok === false ? 'bad' : last.acted ? 'ok' : 'quiet'}">${last.ok === false ? 'one step failed' : last.acted ? 'acted' : 'quiet day'}</span></h1>
...
${pageTail}`;
    ...
    if (path === '/lp/series') {
      ...
      return json({
        ...
        summary: lpSeriesSummary(series, { gas_bnb }),
        points: series,
