...
${jobCensus.providers.slice(0, 40).map((p) => {
  ...
  return `        <tr>
    ...
    <details class="rg-box rg-fold rg-method">
      ...
      <p>Counts: <a href="/api-registry.json">/api-registry.json</a> &middot; every agent that answered, with its tools: <a href="/api-agents.json">/api-agents.json</a>. Both plain JSON, CORS open, so another agent can read them directly. The scanner itself is in <a href="/classic#library">The Library</a> &mdash; run it and check us.</p>
