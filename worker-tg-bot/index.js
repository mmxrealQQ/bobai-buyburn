...
// fifty dollars earned per day in each pool the agent could live in, and the
// pick once every pool has a day of sampled chain. A finding beside the
// numbers, never a move — the agent does not change pools by itself. Empty
// when the record cannot be read, so the card never waits for it.
export function poolRecordLines(pools) {
  ...
  const lines = [`🏁 <b>Pool record</b>  <i>(the same $${Number(pools.usd || 50)} in ±${Number(pools.width_pct || 1)}%)</i>`];
  for (const p of rows) lines.push(`${mark(p.label)} ${p.label}${p.watched ? ' · here' : ''}: ${perDay(p.fees_usd_per_day)}`);
  if (pools.pick) {
    const here = pools.watched && pools.pick.pool === pools.watched.pool;
    lines.push(here
      ? `📌 Pick: <b>${pools.pick.label}</b>, the pool the agent is in`
      : `📌 Pick: <b>${pools.pick.label}</b> — a finding for the operator, not a move`);
  } else {
    const due = pools.pick_due ? ` · due around ${String(pools.pick_due).slice(11, 16)} UTC on ${String(pools.pick_due).slice(5, 10)}` : '';
    lines.push(`📌 No pick yet: every pool needs 24 h of sampled chain (${rows.map((p) => Number(p.hours || 0).toFixed(0) + ' h').join(' / ')})${due}`);
  }
  lines.push('');
  return lines;
}
