...
const categorySections = categorised.map(({ cat, rows }) => {
  ...
  const body = rows.map((r) => {
    ...
    // The facts a buyer weighs, each as one short line that says what it means
    // rather than what it is called. "0 released" is a number; "nobody has been
    // paid out yet" is the same number in a language somebody can act on.
    const facts = [];
    if (q) {
      facts.push(q.quotes
        ? `<li class="rgc-yes">Answers with a price when asked: <b>${esc(q.price || 'a price')}</b></li>`
        : `<li class="rgc-no">Did not answer when we asked it for a price &mdash; ${esc(q.reason || 'no answer')}</li>`);
    }
    if (r.employment) {
      const e = r.employment;
      facts.push(e.funded
        ? `<li>Hired ${fmt(e.funded)} ${e.funded === 1 ? 'time' : 'times'} through the escrow${e.completed ? `, ${fmt(e.completed)} paid out` : ', none paid out yet'}${e.submitted_not_released ? ` (${fmt(e.submitted_not_released)} delivered, still in the dispute window)` : ''}</li>`
        : '<li>Never been hired through the escrow</li>');
    } else if (hireable) {
    ...
    : '<span class="rgc-nohire">Not hireable</span>'}
          ...
          <details class="rgc-ev">
            <summary>How we know this <span class="rg-t rg-${r.hit.source}" title="${esc(why)}">${badge}</span></summary>
  ...
  return `    <div class="rg-box" id="cat-${cat.id}">
      ...
      ${rows.length > 1 ? '<p class="rg-note" style="margin:-8px 0 16px">Ordered by evidence rather than by who runs it: the ones that returned a price when asked come first, then by jobs released, then by jobs funded.</p>' : ''}
...
<style>
  ...
  /* The two or three lines somebody actually decides on. A tick and a dash,
     because "quoted 0.1 $U when asked" and "did not quote" were previously the
     same grey and one of them is the answer to the question being asked. */
  .rgc-facts{list-style:none;margin:0;padding:0;display:grid;gap:4px}
  .rgc-facts li{position:relative;padding-left:16px;font-size:.75rem;line-height:1.55;color:var(--muted);
    overflow-wrap:anywhere}
  .rgc-facts li::before{content:'3';position:absolute;left:0;top:0;color:var(--muted)}
  .rgc-facts li.rgc-yes{color:var(--text)}
  .rgc-facts li.rgc-yes::before{content:'¹3';color:#2ecc71}
  .rgc-facts li.rgc-yes b{color:var(--acc,var(--gold))}
  .rgc-facts li.rgc-no::before{content:' d7';color:#ff6b6b}
