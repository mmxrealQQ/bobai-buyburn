...
// The row IS the amount, at every size: $3,000 is 300 brains, a $400 burn is
// 200 flames. The room is only the ceiling — it matters from the top tiers on
// (a KRAKEN buy past about $7,500, a SUPERNOVA burn past about $1,700), where
// the row stops at the last one that fits. (100ae40 filled the caption from
// the top tier on whatever the size; that was a misreading of the operator's
// sentence, corrected the same day.)
...
// THE PICTURE IS NEVER GIVEN UP FOR THE ROW. The row is sized to the measured
// limit, so this should never fire; it is the net under it. If Telegram ever
// calls a caption too long all the same (its counting changed, a character
// this code counts as one and Telegram as two), the alert is sent again with
// half the row, and again with a quarter, before the text-only fallback is
// even considered. Any other refusal goes straight to that fallback, as before.
export async function sendPhotoWithRow(chatId, photo, row, rest, retry = true) {
  let chars = [...row], shortened = false;
  for (let attempt = 0; attempt < 3; attempt++) {
    const res = await tg('sendPhoto', { chat_id: chatId, photo, caption: `${chars.join('')}\n${rest}`, parse_mode: 'HTML' });
    if (res?.ok === true) return { ok: true, row: chars.join(''), shortened };
    lastPhotoRefusal = res?.description || 'refused';
    if (!retry || !/caption is too long/i.test(lastPhotoRefusal) || chars.length <= 1) break;
    chars = chars.slice(0, Math.floor(chars.length / 2)); shortened = true;
  }
  return { ok: false, row: chars.join(''), shortened };
}
...
🔥 Burned: ${burnedPct}% of supply`;
  ...
  const bar = uncapped ? '🧠'.repeat(Math.max(Math.floor(usdValue / 10), 1)) : buyBar(usdValue, rest);
  let message = `${bar}\n${rest}`;
  ...
  try {
    const sent = await sendPhotoWithRow(chatId, PHOTO_BIGBUY, bar, rest, !uncapped);
    if (sent.ok) {
      console.log('[BUY] photo alert sent', txHash, sent.shortened ? '(row shortened)' : '');
      return 'photo';
    }
    message = `${sent.row}\n${rest}`;
    console.error('[BUY] sendPhoto failed, falling back to text:', lastPhotoRefusal);
  } catch (err) {
...
async function postBurnAlert(newBurned, prevBurned, totalSupply, tokenPrice, chatId = TG_CHAT_ID) {
  try {
...
🔗 <a href="https://bscscan.com/token/${BOBAI_TOKEN}?a=${DEAD}">View Burns</a> · <a href="https://dexscreener.com/bsc/${BOBAI_TOKEN}">Chart</a>`;
    const bar = burnBar(burnedUsd, rest);
    let message = `${bar}\n${rest}`;
    lastPhotoRefusal = null;
    lastCaption = { code_points: [...visibleText(message)].length, utf16_units: visibleText(message).length };
    ...
    // 1) The photo alert
    try {
      const sent = await sendPhotoWithRow(chatId, PHOTO_BURN, bar, rest);
      if (sent.ok) return 'photo';
      message = `${sent.row}\n${rest}`;
      console.error('[BURN] sendPhoto failed, falling back to text:', lastPhotoRefusal);
    } catch (err) {
    ...
    return textRes?.ok === true ? 'text' : false;
  } catch (err) {
...
const BOT_COMMANDS = [
  ...
  { command: 'bot',      description: 'What the bot does — the 3% split now & upcoming' },
  { command: 'burn',     description: 'Burned by the bot & at the dead address, BOBAI and BOB, in USD' },
  { command: 'buy',      description: 'How to buy BOBAI' },
  { command: 'ca',       description: 'Contract address' },
  { command: 'defi',     description: 'The DeFi agent as a portfolio: put in, worth, P&L, last 24 h' },
  ...
  { command: 'worldcup', description: "Worldcup '26 tipgame — final results" },
];
...
const COMMANDS_VERSION = 'v16-bot';
...
// ==================== GIGGLE ACADEMY POT (/giggle) ====================

// The pot's windows — the same instants the buyback worker switches on
// (worker/index.js LP_SHARE_START / GIGGLE_START / GIGGLE_END), so the bot,
// the page and the bot's own split never disagree about when.
const GG_LP_START = Date.parse('2026-09-09T05:30:00Z');
const GG_START = Date.parse('2026-09-17T00:01:00Z');
const GG_END = Date.parse('2026-11-20T00:01:00Z');
const GG_POT_WALLET = '0x5E4102520A71B2AA18a1208330d4848dea4BD105';
const GG_DONATE_URL = 'https://giggleacademy.com/support-us';

// One card, /price-shaped: what it is, where the clock stands, what has been
// collected (from burns.json — giggleBnb/giggleTx per bot run, the same log
// the page reads), the last send, and where the donation goes. Pure.
export function formatGiggleCard(burns, nowMs = Date.now(), bnbUsd = null) {
  const sends = (Array.isArray(burns) ? burns : []).filter((x) => x && x.giggleTx);
  const total = sends.reduce((a, x) => a + (parseFloat(x.giggleBnb) || 0), 0);
  const last = sends.length ? sends[sends.length - 1] : null;
  const span = (ms) => { const d = Math.floor(ms / 86400000), h = Math.floor((ms % 86400000) / 3600000), m = Math.floor((ms % 3600000) / 60000); return d > 0 ? `${d}d ${h}h` : h > 0 ? `${h}h ${m}m` : `${m}m`; };
  let clock;
  ...
  return [
    ...
    '',
    '📅 <b>Window</b>',
    '🗓 Sep 17 → Nov 20, 2026  ·  64 days',
    clock,
    '',
    '⚙️ <b>How it fills</b>',
    '♻️ 0.3% of every trade — a tenth of each tax slice — sent as BNB by the buyback bot',
    '🎁 One donation on UN World Children’s Day, in BNB, through Giggle Academy’s own page',
    '',
    '💰 <b>Collected so far</b>',
    `🪙 <b>${total.toFixed(4)} BNB</b>${bnbUsd > 0 ? ` (≈$${(total * bnbUsd).toFixed(2)})` : ''}  ·  ${sends.length} send${sends.length === 1 ? '' : 's'}`,
    lastLine,
    '',
    `🏦 <a href="https://bscscan.com/address/${GG_POT_WALLET}">Pot wallet</a> · 🎓 <a href="${GG_DONATE_URL}">giggleacademy.com/support-us</a> · 📊 <a href="https://brainonbnb.com/#giggle">brainonbnb.com</a>`,
  ].filter((l, i, a) => l !== '' || a[i - 1] !== '').join('\n');
}

// ==================== WHAT THE BOT DOES (/bot) ====================
// The split as the buyback worker pays it — the same windows as worker/index.js
// and the schedule on the page. Only the windows still ahead or running are
// listed; the finished campaigns live on the page's schedule.
const BB3_START = Date.parse('2026-09-19T18:00:00Z');
// 2026-09-20 06:00 UTC: Liq Boost III raised from 0.3% to 0.5%, the 0.2% out of the BOB burn — the page's schedule (app.js BB3R_START) tells it as two windows, so does this table.
const BB3R_START = Date.parse('2026-09-20T06:00:00Z');
const BOT_WALLET = '0xdeFC0e900Dfc83e207902cF22265Ae63f94c01ce';
const TAX_PHASES_TG = [
  { from: GG_START, until: BB3_START, when: 'Sep 17 → Sep 19, 18:00 UTC, 2026', name: 'Giggle Academy pot + DeFi Agent',
    slices: [
      ['👤', '0.8%', 'Creator', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['👷', '0.8%', 'Burn $BOB (Build On BNB)', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['🧠', '0.8%', 'Burn $BOBAI', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['💧', '0.3%', 'DeFi Agent', ''],
      ['☀️', '0.3%', 'Giggle Academy pot', ''],
    ] },
  { from: BB3_START, until: BB3R_START, when: 'Sep 19, 18:00 UTC → Sep 20, 06:00 UTC, 2026', name: '$BOBAI Liq Boost III + Giggle Academy pot + DeFi Agent',
    slices: [
      ['👤', '0.8%', 'Creator', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['👷', '0.5%', 'Burn $BOB (Build On BNB)', '−0.3% → Liq Boost III, −0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['🧠', '0.8%', 'Burn $BOBAI', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['🔒', '0.3%', 'BOBAI liq add + LP burn', ''],
      ['💧', '0.3%', 'DeFi Agent', ''],
      ['☀️', '0.3%', 'Giggle Academy pot', ''],
    ] },
  { from: BB3R_START, until: GG_END, when: 'Sep 20, 06:00 UTC → Nov 20, 2026', name: '$BOBAI Liq Boost III raised to 0.5% + Giggle Academy pot + DeFi Agent',
    slices: [
      ['👤', '0.8%', 'Creator', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['👷', '0.3%', 'Burn $BOB (Build On BNB)', '−0.5% → Liq Boost III, −0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['🧠', '0.8%', 'Burn $BOBAI', '−0.1% → DeFi Agent, −0.1% → Giggle pot'],
      ['🔒', '0.5%', 'BOBAI liq add + LP burn', ''],
      ['💧', '0.3%', 'DeFi Agent', ''],
      ['☀️', '0.3%', 'Giggle Academy pot', ''],
    ] },
  { from: GG_END, until: null, when: 'from Nov 20, 2026', name: 'Standard',
    slices: [
      ['👤', '1%', 'Creator', ''],
      ['👷', '1%', 'Burn $BOB (Build On BNB)', ''],
      ['🧠', '1%', 'Burn $BOBAI', ''],
    ] },
];
// One card: what the bot is, the split it pays right now — the tax-split box of
// the page, line for line — and the window(s) that follow. Pure.
export function formatBotCard(nowMs = Date.now()) {
  const span = (ms) => { const d = Math.floor(ms / 86400000), h = Math.floor((ms % 86400000) / 3600000), m = Math.floor((ms % 3600000) / 60000); return d > 0 ? `${d}d ${h}h` : h > 0 ? `${h}h ${m}m` : `${m}m`; };
  const idx = TAX_PHASES_TG.findIndex((p) => nowMs >= p.from && (p.until === null || nowMs < p.until));
  const now = idx >= 0 ? TAX_PHASES_TG[idx] : null;
  const later = idx >= 0 ? TAX_PHASES_TG.slice(idx + 1) : TAX_PHASES_TG;
  const lines = (p) => p.slices.map(([e, pct, what, note]) => `${e} <b>${pct}</b> ${what}${note ? `  <i>(${note})</i>` : ''}`);
  const out = [
    '⚙️ <b>What the BOBAI bot does</b>',
    '',
    `🤖 One Cloudflare Worker on wallet <a href="https://bscscan.com/address/${BOT_WALLET}">${BOT_WALLET.slice(0, 6)}…${BOT_WALLET.slice(-4)}</a>, every 10 minutes: trade → 3% tax collects in that wallet → the bot splits it → burns, liquidity, creator, agent, pot. No hand on the switch.`,
    '',
  ];
  if (now) {
    out.push('💸 <b>3% Tax Split — now</b>', `📅 ${now.when} · ${now.name}`, ...lines(now));
    if (now.until !== null) out.push(`⏳ this split ends in <b>${span(now.until - nowMs)}</b>`);
    out.push('');
  }
  if (later.length) {
    out.push('📆 <b>Upcoming</b>');
    for (const p of later) out.push(`📅 ${p.when} · ${p.name}`, ...lines(p), '');
  }
  out.push('🔗 <a href="https://brainonbnb.com/#tokenomics">Full schedule</a> · 🔥 /burn · ☀️ /giggle · 🤖 /defi');
  return out.filter((l, i, a) => l !== '' || a[i - 1] !== '').join('\n');
}

// The burn card's figures: what the bot itself burned (its log), what sits at
// the dead address (chain), and what both are worth today (pair reserves ×
// Chainlink). BOB is token0 in its pair, verified on-chain 2026-09-19.
const BOB_TOKEN = '0x51363f073b1e4920fda7aa9e9d84ba97ede1560e';
const BOB_PAIR = '0x3c79593e01A7f7FeD5d0735B16621e2D52A6bC58';
async function readBobOnchain() {
  try {
    const balanceData = '0x70a08231' + DEAD.slice(2).padStart(64, '0');
    const [deadHex, rHex, bnbUsd] = await Promise.all([
      rpcCall('eth_call', [{ to: BOB_TOKEN, data: balanceData }, 'latest']),
      rpcCall('eth_call', [{ to: BOB_PAIR, data: '0x0902f1ac' }, 'latest']),
      getBnbUsd(),
    ]);
    const burned = deadHex ? Number(hexToBigInt(deadHex)) / 1e18 : null;
    let price = null;
    if (rHex && bnbUsd) {
      const rBob = BigInt('0x' + rHex.slice(2, 66)), rWbnb = BigInt('0x' + rHex.slice(66, 130));
      if (rBob > 0n && rWbnb > 0n) price = (Number(rWbnb) / Number(rBob)) * bnbUsd;
    }
    return { burned, price };
  } catch (e) {
    console.log('[bob] onchain error:', e.message || e);
    return { burned: null, price: null };
  }
}
// Sums of the bot's own burns out of its log. The BOB figure sat under two
// names over time ('bob' first, 'bobBurned' since the Worker) — both count.
export function botBurnsFromLog(burns) {
  const list = Array.isArray(burns) ? burns : [];
  const n = (v) => parseFloat(v) || 0;
  return {
    runs: list.length,
    bobai: list.reduce((a, x) => a + n(x && x.bobaiBurned), 0),
    bob: list.reduce((a, x) => a + n(x && (x.bob || x.bobBurned)), 0),
  };
}
// Billions read as B; the shared formatNumber stops at M because no BOBAI figure reaches 1e9.
const fmtAmt = (n) => n >= 1e9 ? (n / 1e9).toFixed(2) + 'B' : formatNumber(n);
const approxUsd = (amount, price) => price ? ` (≈${formatUsd(amount * price)})` : '';
...
export function formatDefiCard(m, { title = 'DeFi Agent' } = {}) {
  ...
  const blocks = [
    // Dated by the newest check, not the last daily run: a /defi at 02:50
    // UTC shows this morning's worth, so it must not carry yesterday's date.
    [`💧 <b>${title} · ${day(m.checked_at || m.date, true)}</b>`],
    [
...
async function postLpDailyReport(env) {
  ...
  // Each way out says why. On 2026-09-08 the report stayed silent through
  // four ticks after the first 04:23 run while every condition held when
  // replayed from outside, and a function that returns false without a word
  // cannot be told apart from one that never ran.
  const skip = (why) => { console.log('[LP REPORT] skipped: ' + why); return false; };
  if (done === today) return skip('already posted today');
  const r = await fetch('https://agent.brainonbnb.com/lp/agent', { cf: { cacheTtl: 60 } });
  if (!r.ok) return skip('/lp/agent answered ' + r.status);
  ...
  if (!rec.last || String(rec.last.at || '').slice(0, 10) !== today) return skip('no run yet today; last ' + (rec.last && rec.last.at));
  const rs = await fetch('https://agent.brainonbnb.com/lp/series', { cf: { cacheTtl: 60 } });
  if (!rs.ok) return skip('/lp/series answered ' + rs.status);
  ...
  // failed send used to mark it anyway, and the card was lost for the day.
  const sent = await tg('sendMessage', { chat_id: TG_CHAT_ID, text, parse_mode: 'HTML', disable_web_page_preview: true }).catch((e) => ({ ok: false, description: String(e && e.message || e) }));
  if (!sent || !sent.ok) return skip('Telegram refused the card: ' + (sent && sent.description));
...
📈 <a href="https://dexscreener.com/bsc/${BOBAI_TOKEN}">Chart</a> · 🦎 <a href="https://www.geckoterminal.com/bsc/pools/${BOBAI_PAIR}">GeckoTerminal</a>`;
    ...
    case 'burn': {
      ...
      const share = (a, b) => b > 0 ? ` · ${(a / b * 100).toFixed(1)}% of it` : '';
...
<i>Read from the pool contract${lq.block ? ' at block ' + lq.block.toLocaleString('en-US') : ' just now'}.</i>
...
🟪 <a href="https://tradegenius.com/asset/${BOBAI_TOKEN}?network=56">Genius</a>
...
⚙️ /bot — What the bot does: the 3% split now & upcoming
🔥 /burn — Burned by the bot & at the dead address, BOBAI and BOB, in USD
🛒 /buy — How to buy BOBAI
📋 /ca — Contract address
🤖 /defi — The DeFi agent as a portfolio: put in, worth, P&L, last 24 h
☀️ /giggle — Giggle Academy pot: countdown & BNB collected
...
🏆 /worldcup — Worldcup '26 tipgame, final results
...
🖼 <a href="${NFT_DASHBOARD_URL}">brainonbnb.com/nft</a> · 📜 <a href="https://bscscan.com/token/${NFT_CONTRACT}">BscScan</a>`;
    ...
    default: {
      // A slash command the bot does not know (a retired /scan, a typo) gets
      // one line in a private chat; in the group it stays silent, as before.
      if (msg.chat && msg.chat.type === 'private' && text.startsWith('/')) reply = 'I don\'t know that command. /help lists what I can do.';
      break;
    }
  }
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/health' && request.method === 'GET') {
      ...
      return new Response(JSON.stringify({
        ...
        alerts: ['buys', 'burns'],
        // The date each daily post last went out. A running cron says nothing
        // about them: the whale recap was silent for six days in September
        // 2026 under a heartbeat that never missed.
    ...
    if (url.pathname === '/previewbuy' && request.method === 'POST') {
      ...
      if (body.kind === 'burn') {
        const price = 0.00023, prev = 104000000;
        const sent = await postBurnAlert(prev + usd / price, prev, 1000000000, price, OPERATOR_CHAT_ID);
        return new Response(JSON.stringify({ ok: !!sent, kind: 'burn', went_out_as: sent || null, photo_refused: lastPhotoRefusal, caption: lastCaption, usd }), { headers: { 'content-type': 'application/json' } });
      }
      // The longest NFT line an alert really carries, so the preview measures
      // the room a real KRAKEN alert has, not a shorter one.
      const nft = body.nft === false ? '' : '\n🎁 +1 NFT <b>#1925</b> · 🦑 <b>KRAKEN</b> × 🩷 Legendary · 1925/1925';
  ...
  async scheduled(event, env) {
    ...
    // === LP AGENT — one sentence a day ===
    // The daily tick (worker-lp, 04:23 UTC) writes one record; the agent
    // worker serves it at /lp/agent and the series at /lp/series. A few
    // minutes past each tenth minute in the hour after the run, once: the
    // series' own sentence. The per-action alert that used to sit here was
    // asked away on 2026-09-07; its call outlived its function until
    // 2026-09-08 and threw on every tick (caught, logged, harmless, wrong).
    // Gated on the tick's nominal time, not the clock: a tick that starts
    // late, or reaches this line after the awaits above, reads minute 16 from
    // the clock and 15 from the schedule. Every tick in the window says so.
    //
    // Declared out here, not inside the block: the whale gates below read it
    // too. From 2026-09-12 to 09-17 it lived in the block, the gates threw
    // "tick is not defined" into their own catch every minute, and no recap
    // and no 06:00 snapshot went out for six days.
    const tick = nominal;
    {
      // 05:00 UTC since 2026-09-09 — the operator's hour for the public LP
      // card (the whale recap goes at 06:00). The window runs to 07:55: a
      // series point that lands after 05:55 (a late 04:23 run, the agent
      // worker briefly down) used to cost the whole day; the date flag
      // inside keeps it to one card.
      if (tick.getUTCMinutes() % 10 === 5 && tick.getUTCHours() >= 5 && tick.getUTCHours() < 8) {
    ...
    // The window runs to 08:59, like the LP card's: a recap that cannot go out
    // at 06:00 (Telegram down, a deploy mid-hour) still goes out that morning,
    // and the date flag keeps it to one.
    if (TG_INTERNAL_CHAT_ID) {
      try {
        if (tick.getUTCHours() >= 6 && tick.getUTCHours() < 9) {
          const today = tick.toISOString().slice(0, 10);
    ...
    try {
      ...
      if (latestHex) {
        ...
        if (Array.isArray(logs) && logs.length) {
          ...
          // One entry per transaction: a buy split into several swaps is one
          // buy of their sum (foldBuysByTx). BUY = WBNB in AND BOBAI out.
          for (const log of foldBuysByTx(logs)) {
            const txHash = log.transactionHash;
            if (postedSet.has(txHash)) continue;
            const { amount1In, amount0Out } = log;
    ...
    // === CAPTCHA CLEANUP (every tick) ===
    // KV reads only (the index, then one key per open captcha). Every other
    // minute left a joiner who was promised 60 seconds muted for up to three.
