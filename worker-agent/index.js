...
<p class="lead">How wide the DeFi agent sets its price range, and why. Every hour a cron replays a position of $${h(usd)} through the last hour of the CAKE/BNB 0.05% pool and records what each width would have earned; the widths are then replayed over every recorded price with the agent's own re-set delay, its measured re-set cost, and what each re-set lost against holding (a range sells the side that rises and holds the side that falls). The width that nets the most per day is the one the next re-set uses. Nothing here is a forecast.</p>
<h2>The pick</h2>
<div class="card"><dl>
<dt>Width</dt><dd>${pick ? `<b>±${h(pick.width)}%</b> — about $${h(f(pick.earnings.net_usd_per_day, 2))} a day on $${h(usd)} after ${h(pick.earnings.resets)} re-set${pick.earnings.resets === 1 ? '' : 's'} at $${h(f(pick.earnings.reset_cost_usd, 2))} each and $${h(f(pick.earnings.lost_to_price_usd, 2))} lost to the price against holding, over ${h(f(pick.earnings.hours, 0))} h of recorded prices (${h(f(pick.earnings.hours_in_range, 0))} h of them inside the range)` : `${(v.hours_of_prices || 0) >= 24 ? `<b>none</b> — over ${h(v.hours_of_prices || 0)} h of recorded prices no width nets anything once each re-set is charged what its range lost against holding; the agent holds its range instead of re-setting it, and the record keeps measuring` : `none yet — ${h(v.hours_of_prices || 0)} h of prices are on record and 24 h are needed before a width may be picked`}`}</dd>
