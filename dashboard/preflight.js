...
export function shape(s, r, usd, routeError = null) {
  ...
  // ---- the tax
  const t = s.tax || {};
  const taxBuy = r?.transfer_tax?.buy_pct ?? t.buyPct ?? null;
  const taxSell = r?.transfer_tax?.sell_pct ?? t.sellPct ?? null;
  ...
  return {
    ...
    tax: { buy_pct: taxBuy, sell_pct: taxSell, source: r?.transfer_tax?.source || t.source || null },
    depth: { one_percent_buy_usd: d.buyUsd ?? null, one_percent_sell_usd: d.sellUsd ?? null, pool_hard_side_usd: s.pool?.liquidityUsd ?? null },
