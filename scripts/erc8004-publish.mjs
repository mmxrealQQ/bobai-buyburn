...
try {
  ...
  ownAgents = Object.values(own.agents || {}).filter((a) => a.id).map((a) => {
    ...
    const svc = Object.values(SERVICES).find((x) => x.category === a.category);
    return {
      id: a.id,
      name: a.name,
      ...
      speaks: ['a2a', 'x402'],
      // What it can be asked for, named the way the seller names it.
  ...
  const body = rows.map((r) => {
    ...
    return `        <tr${r.ours ? ' class="rg-ours"' : ''}${r.tele ? ` data-tele="${esc(r.tele)}"` : ''}>
          <td><b>${esc(r.label)}</b>${r.ours ? ' <span class="rg-t rg-x4">ours</span>' : ''}${r.instances > 1 ? `<div class="rg-note">${r.instances} registry ids, one deployment</div>` : ''}
              <div class="rg-note">${esc(r.sub)}</div>
              ${(() => {
                const d = describes(r);
                const caps = (r.capabilities || []).slice(0, 6);
                const more = (r.capabilities || []).length - caps.length;
                return `<div class="rg-what${d.weak ? ' rg-weak' : ''}">${d.text ? esc(d.text) : esc(d.why)}</div>`
              ...
              })()}${r.tele ? '<div class="rg-live" hidden></div>' : ''}</td>
          ...
          <td>${hireable ? 'ERC-8183' : '&mdash;'}<div class="rg-note">${hist}</div>${(() => {
            ...
            const mark = q
              ...
              : '';
            return mark + `<button class="rg-hirebtn" data-hire="${r.agentId}" data-name="${esc(r.label)}" data-cat="${cat.id}">Hire &rarr;</button>`;
          })()}</td>
        </tr>`;
  }).join(NL);
...
// How many of those actually returned a price the last time they were asked.
// Published rather than only rendered, so a reader of the JSON gets the same
// caveat as a reader of the page.
if (hireConfirm) {
  api.quoted_when_asked = (hireConfirm.agents || []).filter((a) => a.quotes).length;
...
const deployments = (() => {
  ...
  return { rows, total, top5, singles, biggestFleet, hosts: rows.length };
})();
    ...
    <div class="rg-box rg-start">
      ...
      <div class="rg-do">
        ...
        <div class="rg-do-c">
          ...
          <p class="rg-note">${fmt(api.hireable_here)} carry a Hire button${hireConfirm ? `, and <b>${fmt(api.quoted_when_asked)} of them returned a price</b> the last time each one was actually asked` : ''}. The quote is negotiated live with the agent and the escrow runs on-chain over ERC-8183. Every row says how it was categorised, whether it has ever been paid, and what it answered when asked.</p>
        </div>
        <div class="rg-do-c">
...
${categorySections}
    ...
    <details class="rg-box rg-fold" id="rg-fleets">
      <summary><h2>What the ones that answer actually are</h2><span class="rg-peek">${fmt(deployments.total)} ids &rarr; ${fmt(deployments.hosts)} deployments</span></summary>
      ...
      <div class="rg-grid">
        ...
        <div class="rg-card"><div class="rg-n">${fmt(deployments.hosts)}</div><div class="rg-l">distinct deployments</div><div class="rg-s">what is actually running</div></div>
...
${deployments.rows.slice(0, 40).map((d) => `        <tr>
      ...
      <p class="rg-note" style="margin-top:12px">${deployments.rows.length > 40 ? `Showing the 40 largest of ${fmt(deployments.rows.length)} deployments; the remainder hold one or two ids each. ` : ''}Grouped by the host each registration names. Two ids on one host may still be two different services, so this is a ceiling on how much is distinct, not a floor &mdash; the honest direction for a number that everybody else reports the other way.</p>
    </details>
...
${jobCensus.providers.slice(0, 40).map((p) => {
  ...
  return `        <tr>
    ...
    <details class="rg-box rg-fold rg-method">
      ...
      ${hireConfirm ? `<p><b>Whether it will quote.</b> Every agent carrying a Hire button was sent the ERC-8183 negotiation step once &mdash; a request for a price on the service it is listed under, and nothing else. ${fmt(hireConfirm.quoted)} of ${fmt(hireConfirm.asked)} returned one; the rest answered with unparseable JSON, an error from their own infrastructure, or a schema complaint, and each row carries which. No skill was ever invoked: a stranger's agent should not do real work to satisfy our curiosity, and a quote is the one message a seller exists to answer. Measured ${esc((hireConfirm.measured_at || '').slice(0, 16).replace('T', ' '))} UTC.</p>` : ''}
...
<footer><div class="fi2">
  ...
  <div class="fm">
    ...
    <p style="margin-top:6px;opacity:.75">Made by <a href="/">Brain On BNB AI</a> &middot; <a href="/whitepaper">Whitepaper</a> &middot; <a href="/advantage">Advantage report</a> &middot; <a href="/session">Agent spending authority</a> &middot; not financial advice</p>
  </div>
</div></footer>
