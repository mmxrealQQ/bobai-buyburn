...
if (process.argv.includes('--self-test')) {
  ...
  is('out of range with a width named: the sentence says when the re-set comes, how wide, and that it trades nothing', /^Out of range for 2\.0 h\. Re-set after 3 h out of range: the new range one-sided beside the price, ±2% wide, no trade\.$/.test(lpPortfolio(rec, series, { now: NOW, width: w2, outsideSince: new Date(NOW - 2 * 36e5).toISOString() }).next));
  is('a wait of zero reads as the first hourly check, not "after 0 h"', (() => { const w0 = { width_pct: 5, wait_hours: 0 }; const t = lpPortfolio(rec, series, { now: NOW, width: w0, outsideSince: new Date(NOW - 2 * 36e5).toISOString() }).next; return /at the first hourly check that finds it out of range/.test(t) && !/after 0 h/.test(t); })());
