// BOBAI Tax Distribution Bot — Cloudflare Worker
// Runs natively on a Cloudflare Cron Trigger every 10 minutes (no GitHub dependency):
// 1. Checks buyback wallet for BNB from BOBAI 3% tax
// 2. Splits BNB per active phase windows (see below)
// 3. Buys & burns $BOB and $BOBAI, adds perma-liquidity during boost phases
// Logs go to Workers KV and are served at https://logs.brainonbnb.com/logs/<file>
//
...
// Supabase REST helper (used to record WC26 TAX adds — inactive outside WC26 window).
...
// KV log helper: append one entry to a JSON-array log stored in KV.
//
// A LOG ENTRY THAT DID NOT GET WRITTEN (2026-09-19). The log is one array,
// read, extended and written back, and a failure here was printed and gone:
// the money had moved, and the burn table and the Giggle card counted too
// little for good. It runs after the last transfer, so nothing it does can
// touch money. Three tries; an entry that still cannot be written is parked
// under a key of its own (a few hundred bytes instead of the whole array) and
// the next append carries it in, oldest first. /health counts what is parked.
// Same text in worker-dev-buyback/index.js.
const UNLOGGED = 'unlogged:';
async function kvAppend(env, key, entry, pauseMs = 2000) {
  let last;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const cur = await env.LOGS.get(key);
      const arr = cur ? JSON.parse(cur) : [];
      if (!Array.isArray(arr)) throw new Error('the stored log is not an array');
      const parked = (await env.LOGS.list({ prefix: `${UNLOGGED}${key}:` })).keys.map(k => k.name).sort();
      // A put that threw may still have landed, and a parked key may have
      // outlived its entry: nothing goes in twice (time is the identity).
      const have = new Set(arr.map(e => e && e.time));
      for (const name of parked) {
        const raw = await env.LOGS.get(name);
        const e = raw ? JSON.parse(raw) : null;
        if (e && !have.has(e.time)) { arr.push(e); have.add(e.time); }
      }
      if (!have.has(entry.time)) arr.push(entry);
      if (parked.length) arr.sort((a, b) => String(a.time).localeCompare(String(b.time)));
      await env.LOGS.put(key, JSON.stringify(arr, null, 2));
      for (const name of parked) await env.LOGS.delete(name).catch(() => {});
      console.log(`Logged to KV: ${key}${parked.length ? ` (with ${parked.length} parked)` : ''}`);
      return true;
    } catch (e) {
      last = e;
      console.log(`Failed to log ${key} (try ${attempt} of 3): ${e.message}`);
      if (attempt < 3) await new Promise(r => setTimeout(r, pauseMs * attempt));
    }
  }
  try {
    await env.LOGS.put(`${UNLOGGED}${key}:${entry.time}`, JSON.stringify(entry));
    console.log(`Parked the entry for the next append: ${UNLOGGED}${key}:${entry.time}`);
  } catch (e) {
    console.log(`ENTRY LOST, could not even park it (${e.message}; log error: ${last && last.message}): ${JSON.stringify(entry)}`);
  }
  return false;
}
...
async function runBot(env) {
  const privateKey = env.BUYBACK_PRIVATE_KEY;
  if (!privateKey) {
    console.log('[ERROR] No BUYBACK_PRIVATE_KEY set');
    return;
  }
  ...
  if (bobLiqResult) {
    await kvAppend(env, 'liq-boost-log.json', {
      time: new Date().toISOString(),
  ...
  // Step 8: Log BOBAI liq boost (same shape)
  if (bobaiLiqResult) {
    await kvAppend(env, 'bobai-liq-log.json', {
      time: new Date().toISOString(),
...
// Files served via /logs/<name> — the public burn/liq history.
const SERVED_LOGS = new Set([
  'burns.json',
  'bobai-liq-log.json',
  'liq-boost-log.json',
  'dev-buyback-log.json',
]);
...
export default {
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      await env.LOGS.put('heartbeat-buyback', new Date().toISOString());
      await env.LOGS.delete('lock-buyback');
    }
  },
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      // `unlogged`: entries a bot could not write into its log and parked for
      // the next append (kvAppend). Anything but an empty list is a run whose
      // money moved and whose line is not in the public log yet.
      const [hb, hbDev, parked] = await Promise.all([
        env.LOGS.get('heartbeat-buyback'),
        env.LOGS.get('heartbeat-dev'),
        env.LOGS.list({ prefix: UNLOGGED }).then(l => l.keys.map(k => k.name.slice(UNLOGGED.length)), () => null),
      ]);
      return new Response(JSON.stringify({ buyback: hb, devBuyback: hbDev, unlogged: parked }), {
        headers: { 'Content-Type': 'application/json', ...CORS_HEADERS },
      });
    }

    // Serve log files from KV: /logs/<name>
    const m = url.pathname.match(/^\/logs\/([a-z0-9-]+\.json)$/);
    if (m && SERVED_LOGS.has(m[1])) {
      const body = await env.LOGS.get(m[1]);
      if (body === null) return new Response('[]', { headers: { 'Content-Type': 'application/json', ...CORS_HEADERS } });
