...
<dt>Wait</dt><dd>${(() => { const dt = v.delay_test || {}; const ds = dt.delays || []; if (!ds.length) return `${h(dt.in_use_hours ?? 2)} h outside the range before a re-set — the wait the agent uses; whether another wait would net more is replayed once a day of prices is on record`; const line = ds.map((d) => `${h(d.hours)} h: ${d.net_usd_per_day == null ? 'nothing' : `$${h(f(d.net_usd_per_day, 2))} a day at ±${h(d.width)}% after ${h(d.resets)} re-set${d.resets === 1 ? '' : 's'}`}${d.in_use ? ' (in use)' : ''}`).join(' · '); const p = dt.pick; return `<b>${h(dt.in_use_hours)} h</b> outside the range before a re-set is what the agent uses. Replayed with every wait: ${line}.${p && !p.in_use ? ` The record favours ${h(p.hours)} h; the agent keeps ${h(dt.in_use_hours)} h until the operator changes it.` : p ? ' The wait in use is the one that netted the most.' : ''}`; })()}</dd>
