...
export function formatScan(d, address) {
  ...
  // A token still raising on four.meme has no pool by design; the API answers
  // from the platform's own contract instead, and so does this.
  if (d.curve && d.curve.platform === 'four.meme') {
    const cv = d.curve;
    const pct = (n) => (n == null ? '?' : Number(n).toFixed(2) + '%');
    const q = cv.quoteSymbol || 'quote';
    const amt = (n) => (n == null ? '?' : Number(n) >= 100 ? Math.round(Number(n)).toLocaleString('en-US') : Number(n).toFixed(2)) + ' ' + q;
    const costs = Array.isArray(cv.tradeCost) ? cv.tradeCost.filter((t) => [100, 500, 1000].includes(t.sizeUsd)) : [];
    const costLines = costs.map((t) => `$${t.sizeUsd} buy costs <b>${t.buyCostPct == null ? (t.note || 'not quoted') : pct(t.buyCostPct)}</b> · sell ${t.sellCostPct == null ? 'not quoted' : pct(t.sellCostPct)}`).join('\n');
    const priceStr = cv.priceUsd == null ? amt(cv.priceQuote) : '$' + Number(cv.priceUsd).toPrecision(4);
    return `🔍 <b>${d.symbol || '?'}</b> ${d.name && d.name !== d.symbol ? `(${d.name}) ` : ''}— still on the <b>four.meme launch curve</b>, no PancakeSwap pool yet
💰 Price ${priceStr} · Raised <b>${amt(cv.raised)}</b> of ${amt(cv.maxRaising)} (${pct(cv.progressPct)})
🏦 The money raised sits in four.meme's contract until the raise completes — no pool, so no liquidity to pull
${cv.sellQuoted ? '✅ Sell quote: four.meme would pay out a sell right now' : '❔ Sell quote: the curve quoted no sell just now'}

<b>What a trade really costs on the curve</b> <i>(${pct(cv.feePct)} platform fee + the curve moving)</i>
${costLines || 'no size quotable right now'}

📊 <a href="${link}">Full scan</a>
<i>Read from four.meme's helper contract on BNB Smart Chain just now. Measurement, not advice.</i>`;
  }
  ...
  // The sell test: the scanner's own simulation on the chain, this block.
  const sim = d.sellability || null;
  const sellLine = !sim ? null
    : !sim.ok ? `🧪 Sell test: not run — ${String(sim.reason || '').slice(0, 90)}`
    : sim.sellable ? '✅ Sell test: a sell goes through right now (simulated on-chain, fresh address)'
    : `🚫 <b>Sell test: the sell REVERTED</b> — ${String(sim.sell_error || 'no reason given').slice(0, 90)}. Do not buy what you cannot sell.`;
