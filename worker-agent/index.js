...
// THE SPEC, READ BEFORE ANY MONEY MOVES (2026-09-24, A7 step 1). The 402's
// own example had no pair, and the paid call required one: a trader who
// followed it was refused after paying for nothing — the order was right
// (400 before the charge), the example led there. And quote, callback and
// threshold were only checked inside createWatch, after the payment, so a
// typo in the callback cost a credit round trip. Now: pair is optional —
// the token's PancakeSwap V2 pair with the quote (WBNB by default) is looked
// up at the V2 factory — and a pair that is given must answer getReserves,
// i.e. be a V2 pair (the watch reads reserves; a V3 pool would read nothing),
// holding this token. Every refusal comes before the payment.
const V2_FACTORY = '0xca143ce32fe78f1f7019d7d551a6402fc5350c73';
const addrArg32 = (a) => String(a).toLowerCase().replace(/^0x/, '').padStart(64, '0');
async function resolveWatchSpec(spec) {
  const out = { ...(spec || {}) };
  if (!/^0x[a-fA-F0-9]{40}$/.test(out.token || '')) return { error: 'token must be a BSC address (0x…)' };
  const quote = String(out.quote || WBNB).toLowerCase();
  if (!WATCH_QUOTES.has(quote)) return { error: 'this watch prices pools quoted in BNB, USDT, BUSD, USDC or USD1 — name one of those as quote, or leave it out for BNB' };
  if (out.callback != null && !/^https:\/\/[^\s]{4,400}$/.test(String(out.callback))) return { error: 'callback must be an https:// URL' };
  if (out.depthBelowUsd != null && !(Number(out.depthBelowUsd) > 0)) return { error: 'depthBelowUsd must be a positive number of dollars' };
  const token = out.token.toLowerCase();
  let pair = out.pair ? String(out.pair).toLowerCase() : null;
  if (pair && !/^0x[a-f0-9]{40}$/.test(pair)) return { error: 'pair must be a BSC address (0x…), or left out to be found from the token' };
  if (!pair) {
    const got = await call(V2_FACTORY, '0xe6a43905' + addrArg32(token) + addrArg32(quote)).catch(() => null);
    const found = got && got.length >= 66 ? '0x' + got.slice(-40).toLowerCase() : null;
    if (!found || /^0x0{40}$/.test(found)) return { error: 'this token has no PancakeSwap V2 pair with that quote — the watch reads V2 reserves; name a V2 pair, or another quote' };
    pair = found; out.pair_found = true;
  }
  const res = await call(pair, SEL.getReserves).catch(() => null);
  if (!res || res.length < 2 + 64 * 2) return { error: 'that pair does not answer as a PancakeSwap V2 pair (no reserves) — the watch reads V2 reserves, a V3 or Infinity pool cannot be watched' };
  const t0 = await call(pair, SEL.token0).catch(() => null);
  const t1 = await call(pair, '0xd21220a7').catch(() => null);
  const sides = [t0, t1].map((x) => (x && x.length >= 66 ? '0x' + x.slice(-40).toLowerCase() : null));
  if (!sides.includes(token)) return { error: 'that pair does not hold this token' };
  if (!sides.includes(quote)) return { error: 'that pair is not quoted in the quote named (WBNB by default) — name its quote' };
  return { spec: { ...out, token, pair, quote } };
}
...
const WATCH_TOOL = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      pair: { type: 'string', pattern: '^0x[a-fA-F0-9]{40}$', description: 'Optional. The PancakeSwap V2 pair holding it; left out, it is found from the token and the quote.' },
...
async function purchaseWatch(env, ctx, payTo, spec, proof) {
  ...
  const ignored = Object.keys(spec || {}).filter((k) => !KNOWN.has(k) && k !== 'pair_found');

  return { status: 200, body: {
    ok: true,
    watch: watch.id,
    expires: new Date(watch.expiresAt).toISOString(),
    watching: { token: watch.token, pair: watch.pair, depthBelowUsd: watch.depthBelowUsd, ...(spec && spec.pair_found ? { pair_note: 'found from the token: its PancakeSwap V2 pair with ' + (watch.quote === WBNB ? 'WBNB' : watch.quote) } : {}) },
...
async function buildLpSeries(env) {
      ...
      return {
        ...
        cadence: 'daily, after the 04:23 UTC run; the range itself is checked every hour, and a run between them — an hourly check or the deposit watch — gets a point of its own when it acted (a re-set, a top-up from a deposit) or found a position the series did not know. A run that found no position is not a point',
      };
}
...
<dt>Re-set cost</dt><dd>$${h(f(v.reset_cost && v.reset_cost.usd, v.reset_cost && Number(v.reset_cost.usd) > 0 && Number(v.reset_cost.usd) < 0.01 ? 4 : 2))} per $50 — ${h(v.reset_cost && v.reset_cost.basis)}</dd>
...
<dt>Measured</dt><dd>${v.calibration ? `the agent's own position at ±${h(v.calibration.position_width_pct)}% earned <b>$${h(f(v.calibration.measured_usd_per_day_on_50, 2))} a day on $50</b> over the last ${h(f(v.calibration.hours, 0))} h, against $${h(f(v.calibration.replay_usd_per_day_on_50, 2))} the replay puts on that width${v.calibration.replay_hours != null ? ` over its own ${h(f(v.calibration.replay_hours, 0))} h` : ''}${v.calibration.factor != null ? ` — ${h(f(v.calibration.factor * 100, 0))}% of the replay's figure` : ''}. ${v.calibration.replay_is === 'above' ? 'Here the replay says more than the position earned' : v.calibration.replay_is === 'below' ? 'Here the position earned more than the replay says' : v.calibration.replay_is === 'close' ? 'Here the two are within a tenth of each other' : 'The two cannot be set against each other yet'}; the two cover different hours, so this is a check on the dollars, not a correction. The pick compares widths with each other on the same replay and is not scaled; the dollar beside it is an estimate, this line is the measurement.` : 'the position has not earned for a day yet on the series — the replay\'s dollars are estimates until it has'}</dd>
...
<div class="card"><div class="wrap"><table>
<tr><th>When</th><th>Left</th><th>Into</th><th>Price moved</th><th>Position</th><th>Lost to the price</th><th>Execution</th><th>Impact</th></tr>
${v.resets.rows.map((r) => `<tr><td>${h(when(r.at))}${r.forced_by_deposit ? ' · deposit' : ''}</td><td>±${h(r.from_width_pct)}%</td><td>${r.to_width_pct != null ? '±' + h(r.to_width_pct) + '%' : '—'}</td><td>${r.price_move_pct > 0 ? '+' : ''}${h(f(r.price_move_pct, 2))}%</td><td>${h(f(r.position_bnb, 4))} BNB</td><td>${h(f(r.lost_to_price_bnb, 5))} BNB (${h(f(r.lost_to_price_pct, 2))}%)${r.lost_to_price_usd != null ? ' ≈ $' + h(f(r.lost_to_price_usd, 2)) : ''}</td><td>${h(f(r.execution_bnb, 6))} BNB${r.execution_usd != null ? ' ≈ $' + h(f(r.execution_usd, 2)) : ''}</td><td>${r.impact_bnb == null ? 'not measured' : h(f(r.impact_bnb, 6)) + ' BNB'}</td></tr>`).join('')}
</table></div>
<p class="note">${h(v.resets.basis.replace(/^[a-z]/, (ch) => ch.toUpperCase()))}. The loss to the price is what the earnings test charges every replayed re-set; here it is read off the re-sets that happened.</p></div>` : ''}
...
${pageTail}`;
    ...
    if (path === '/lp/agent' || path === '/lp/collect') {
      ...
      const histRows = hist.slice().reverse().slice(0, 60).map((e) => {
        ...
        // New BNB in, as the sums count it; an increase that only put the
        // wallet's leftovers back in brought no new BNB and says what it put.
        if (s.increase?.acted && !s.increase.error) parts.push(increaseIntoPosition(s.increase) >= 0.00001 || !(Number(s.increase.other_used) > 0)
          ? `added ${f(increaseIntoPosition(s.increase), 5)} BNB to the position`
          : `put the wallet's leftover ${f(s.increase.other_used, 4)} ${String(s.increase.would_add?.other_token || '').toLowerCase() === '0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82' ? 'CAKE' : 'of the other token'}${Number(s.increase.wbnb_used) > 0 ? ` and ${f(s.increase.wbnb_used, 5)} WBNB` : ''} into the position — no new BNB`);
...
<div><b>Waiting</b><span>${flow.waiting.income.length ? flow.waiting.income.map((w) => `${f(w.amount, 2)} ${h(w.token)} on the ${h(w.source || 'income')} wallet`).join(', ') : 'no income on the wallets'}; ${flow.waiting.fees_owed_bnb == null ? 'fees owed by the position not read at the last run' : `${f(flow.waiting.fees_owed_bnb, 6)} BNB of fees owed by the position`}${flow.waiting.wallet_spendable_bnb != null ? `; ${f(flow.waiting.wallet_spendable_bnb, 5)} BNB in the DeFi wallet above the reserve` : ''}</span><i>Each moves once it is worth more than the gas it costs.</i></div>
...
<p class="note">Quiet days are not listed; the last one is always above. Same facts as JSON: <a href="/lp/agent?format=json">/lp/agent?format=json</a> · width record: <a href="/lp/windows">/lp/windows</a> · pool record: <a href="/lp/pools">/lp/pools</a> · <a href="https://brainonbnb.com/liquidity">Back to the liquidity page</a></p></div>
${pageTail}`;
      return new Response(html, { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-store' } });
    }
    ...
    if (path === '/mcp') {
      ...
      if (method === 'tools/call') {
        ...
        // The same reading of the spec as the HTTP door, before any payment
        // (A7): pair optional, found from the token; every refusal first.
        const ready = await resolveWatchSpec(spec0).catch(() => ({ error: 'the pair could not be read right now' }));
        if (ready.error) {
          return rpcOk(id, {
            isError: true,
            content: [{ type: 'text', text: ready.error + (payment ? ' Nothing was charged.' : '') }],
          });
        }
    ...
    if (path === '/watch' && request.method === 'POST') {
      ...
      if (!proof) {
        ...
        note('sell:watch:terms');
        // A buyer who already named the token sees, before paying, which pair
        // would be watched — or why none can be (2026-09-24, A7).
        if (spec && spec.token && out.body && typeof out.body === 'object') {
          const ready = await resolveWatchSpec(spec).catch(() => ({ error: 'the pair could not be read right now' }));
          out.body.would_watch = ready.error ? { refused: ready.error } : { token: ready.spec.token, pair: ready.spec.pair, quote: ready.spec.quote, pair_found: !!ready.spec.pair_found };
        }
        return json(out.body, out.status, out.headers || {});
      }
      ...
      const ready = await resolveWatchSpec(spec);
      if (ready.error) return json({ error: ready.error, payment: 'not taken — nothing was charged' }, 400);

      const out = await purchaseWatch(env, ctx, payTo, ready.spec, proof);
