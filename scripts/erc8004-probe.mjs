...
// protocolVersion or a skills array. That is much looser than the standard and
// it over-counted. An A2A card names the URL you POST JSON-RPC to and the
// skills you can ask for; a document with neither is a description of an agent,
// not an agent you can reach.
//
// Audited on 2026-08-25 across every host behind our A2A-flagged agents: 261 of
// 290 served a card naming both, 29 did not. One of the 29 is a paid REST
// catalogue on Arbitrum that publishes an agent.json — we had it filed as an
// A2A agent on BNB Chain.
//
// Both facts are kept. `card` means a document was served, which is worth
// knowing; `a2a` now means callable. Collapsing them was the original mistake
// and merging them again would repeat it.
const cardIsCallable = (j) => !!(
  j && typeof j === 'object'
  && typeof j.url === 'string' && /^https?:\/\//i.test(j.url)
  && Array.isArray(j.skills) && j.skills.some((s) => s && typeof s === 'object' && (s.id || s.name))
);
...
async function probeAgent(a) {
    ...
    if (/agent-card\.json$/i.test(url) || /\/\.well-known\//i.test(url)) {
      try {
        ...
        if (j && (j.name || j.protocolVersion || j.skills)) {
          result.live.card = true;
          result.live.a2a = cardIsCallable(j);
  ...
  if (result.reachable && !result.live.card) {
      ...
      for (const p of ['/.well-known/agent-card.json', '/.well-known/agent.json', '/.well-known/ai-agent.json']) {
        ...
        try {
          ...
          if (j && (j.name || j.protocolVersion || j.skills)) {
            ...
            if (typeof j.url === 'string') result.live.a2aUrl = j.url;
            if (Array.isArray(j.skills)) {
              result.live.skills = j.skills
                .map((s) => (s && typeof s === 'object' ? s.name || s.id : s))
...
let done = 0, reachable = 0, withMcp = 0, withA2a = 0, withCard = 0;
...
await Promise.all(Array.from({ length: CONCURRENCY }, async () => {
  while (queue.length) {
    ...
    if (r.live.card) withCard++;
    if (done % 25 === 0) console.log(`  ${done}/${todo.length}  reachable ${reachable}  mcp ${withMcp}  a2a ${withA2a}  cards ${withCard}`);
  }
}));
out.end();
...
const census = {
  ...
  endpoints: {
    ...
    // Two different facts, kept apart. A served card means a document exists;
    // a callable one names the endpoint you POST to and the skills you can ask
    // for. Ten percent of what we used to call A2A had the first and not the
    // second — see scripts/a2a-card-audit.mjs.
    serving_an_agent_card: withCard,
    a2a_callable: withA2a,
  },
...
console.log(`  served an agent card  ${withCard}`);
console.log(`  of those, callable   ${withA2a}  (card names both an endpoint and skills)`);
if (scan.highestId) {
