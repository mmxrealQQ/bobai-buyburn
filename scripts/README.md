...
| `brainscreener-regressions.cjs` | brainScreener's scoring, 25 pins from the two reviews of 2026-09-20 (EAT-26 subscales and grading, PHQ-9 item 9 and the crisis box, IQ percentile and answer positions, WHODAS "Not applicable", the childhood scale's threshold of 36, and that no page, result text or llms.txt lists the WURS-K as an instrument used — the childhood scale is based on it); offline, part of `self-tests.mjs`. Run it after any change under `dashboard/brainscreener/assets/js/` |
| `preflight-regressions.mjs` | what `bsc_token_preflight` (`dashboard/preflight.js`, 2026-09-20) makes of a pool scan and a route answer: a sell that does not go through stops, a simulation that did not run is "not checked" and never "sellable", an unknown tax stays null and never 0, the lines for a high tax / a thin unburned LP / a size above the 1% depth, the cost ladder read between its rungs when no route answers, the four.meme curve, no "safe" and no score. 15 pins, offline; part of `self-tests.mjs`. Run it after any change to `preflight.js` or to the fields of the scan or the route it reads |
| `stats-detail-regressions.mjs` | the "what was asked for" counters behind `agent.brainonbnb.com/stats/detail` (2026-09-20): what `dashboard/_worker.js` posts to `/hit` for an MCP method and tool, a page of ours against a caller from outside, a crawler inventing routes and tool names (a path we do not serve is `rest:unknown`, whatever referer it carries) — and on the agent worker one KV key per isolate and day, names cleaned and capped, the smoke test left out, nothing of it in `count:*` (the public totals of `/stats`). 25 pins, offline, runs the real worker code; part of `self-tests.mjs`. Run it after any change to the counting in either worker |
...
| `x402-permit2-regressions.mjs` | the reading of a standard x402 Permit2 payment (shared/x402-permit2.js), offline, both ways: price, token, payee, spender, a life long enough for the queue, the settle calldata, the receipt's transfer. self-tests runs it |
| `x402-permit2-fork-test.mjs` | the x402 Permit2 queue RUN on a local fork (anvil, archive RPC in `BSC_RPC_KEYED_URL_2`, two throwaway keys, nothing leaves 127.0.0.1): a buyer swaps, approves Permit2 and signs as the official client does; worker-agent's door, then worker-lp's settleX402Queue: settled, a replay, a revoke after the answer, too little, too short, expired in the queue, a run cut off after sending. Run it after any change to shared/x402-permit2.js or worker-lp/x402-settle.js |
