...
async function kvAppend(env, key, entry) {
  try {
    let arr = [];
    const cur = await env.LOGS.get(key);
    if (cur) arr = JSON.parse(cur);
    arr.push(entry);
    await env.LOGS.put(key, JSON.stringify(arr, null, 2));
    console.log(`Logged to KV: ${key}`);
  } catch (e) {
    console.log(`Failed to log ${key}: ${e.message}`);
  }
}
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      const [hb, hbDev] = await Promise.all([
        env.LOGS.get('heartbeat-buyback'),
        env.LOGS.get('heartbeat-dev'),
      ]);
      return new Response(JSON.stringify({ buyback: hb, devBuyback: hbDev }), {
