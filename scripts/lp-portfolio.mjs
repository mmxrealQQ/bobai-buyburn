...
if (process.argv.includes('--self-test')) {
  ...
  const series = {
    summary: {
      ...
      deposits_put_in_bnb: 0.1358, income_put_in_bnb: 0, fees_owed_now_bnb: 0.00087, bobai_held_units: 3895.4, fees_into_bobai_bnb: 0.001093,
  ...
  const m = lpPortfolio(rec, series, pools, { now: NOW });
  ...
  is('the last day lists the runs of the last 24 h only, oldest first', m.last_24h.length === 2 && m.last_24h[0].step === 'increase' && m.last_24h[1].step === 'rebalance' && /7397034/.test(m.last_24h[1].what));
  is('a re-set that bought BOBAI says so', /into \$BOBAI/.test(m.last_24h[1].what));
