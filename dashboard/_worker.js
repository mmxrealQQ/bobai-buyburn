...
const AGENT_GUIDE = {
  ...
  what_this_is: 'Read-only measurement for ANY token on BNB Chain (BSC), for agents about to act: call bsc_token_preflight before a trade (can I get in, can I get out, at your size), bsc_pool_scan and pancakeswap_best_route for the figures behind it, the ERC-8004 agent registry and census, and live on-chain data for $BOBAI, the token of the project that runs it. Not a trading bot — nothing here signs or moves funds.',
  what_you_can_know: [
  ...
  what_costs_money: [
    { what: 'Keep watching a pool after a preflight: a callback when the size that moves its price 1% falls below yours', where: 'POST https://agent.brainonbnb.com/watch — over MCP: bsc_pool_watch at https://agent.brainonbnb.com/mcp' },
    { what: 'The measured plans sold per answer (grid, yield, rebalance, health factor, LP tier, LP position)', where: 'GET https://agent.brainonbnb.com/answer lists them with their inputs; a free example of each at GET /example?service=…' },
    { note: 'Sent without payment, each answers 402 with its price. Every tool on this server is free.' },
  ],
  guided_prompts: [
...
const TAX_PHASES = [
  ...
  { id: 'bobai-liq-3', from: '2026-09-19T18:00:00Z', until: '2026-09-20T06:00:00Z', split_pct: { creator: 0.8, bob_burn: 0.5, bobai_burn: 0.8, bobai_liquidity_add: 0.3, defi_agent: 0.3, giggle_academy_pot: 0.3 } },
  { id: 'bobai-liq-3-raised', from: '2026-09-20T06:00:00Z', until: '2026-11-20T00:01:00Z', split_pct: { creator: 0.8, bob_burn: 0.3, bobai_burn: 0.8, bobai_liquidity_add: 0.5, defi_agent: 0.3, giggle_academy_pot: 0.3 } },
  ...
  try {
    if (method === 'initialize') {
      ...
      return new Response(JSON.stringify(rpcOk(id, {
        ...
        serverInfo: { name: 'Brain On BNB AI ($BOBAI)', version: '1.4.0' },
        // Read by MCP clients as the server's own introduction (2026-09-24,
        // A4 of the review): without it a client saw a $BOBAI data feed and
        // never learned the tools work for every BSC token.
    ...
    if (method === 'tools/call') {
      ...
      // A tool that answers "that is not a BSC token" has answered: MCP wants
...
const A2A_CARD = {
  ...
  // WHO THIS IS, ON CHAIN (2026-09-19). Agent #49467 names THIS card as its
  // document, and the card said nothing of it: no registrations, no trust model,
  // no registry to check either against — the gap Brain Plaza marks in other
  // operators' cards, in our own front door. The proof document
  // (/.well-known/agent-registration.json) is what a verifier fetches AFTER it
  // knows which ids to check; a card is what a client reads FIRST. Same list,
  // same registries as the agent origin's card, from one file
  ...
  skills: [
    ...
    { id: 'token_preflight', name: 'Before any trade: one check at your size',
      description: 'For an agent about to trade any token on BNB Chain. One call: what stops the trade (the sell does not go through from a fresh address, nothing quotes, half the money gone on a round trip), what to weigh (tax high, unknown or changeable; a size that moves the price; LP that can be withdrawn; contract flags — each with its figure and the line it was measured against), then the best route, the slippage in bps this size needs and the round trip with the measured transfer tax. About 2.5 KB; no "safe", no score. MCP: bsc_token_preflight. REST: /api/preflight?address=0x…&usd=250',
    ...
    // What is sold, listed among the free tools on purpose: a card that
    // advertises only what costs nothing leaves an agent to discover the paid
    // services by accident, and a price belongs in the first sentence so that
    // deciding whether to call never requires calling. This comment said 'the
    // one paid skill' until 2026-09-19, months after the answers sold per call
    ...
    { id: 'paid_answers', name: 'Answers sold per call (paid)',
      ...
      tags: ['defi', 'bsc', 'x402', 'erc-8183', 'a2a', 'negotiate', 'paid'] },
