...
showClear();
// THE WAY IN FOR SOMEBODY WITH NO ADDRESS. Three tokens anyone on BNB Chain
// has heard of, plus whatever this visitor scanned before (kept in this
// browser only, never sent anywhere). Without this the page is a headline
// and an empty box, and the second visit starts from nothing again.
const TRY=[['CAKE','0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82'],['USD1','0x8d0d000ee44948fc98c9b98a4fa4921476f08b0d'],['BOBAI','0x245c386dcfed896f5c346107596141e5edcbffff']];
const RECENT_KEY='sc-recent';
function recent(){try{const r=JSON.parse(localStorage.getItem(RECENT_KEY)||'[]');return Array.isArray(r)?r.filter(x=>x&&/^0x[0-9a-f]{40}$/i.test(x.a)):[]}catch(e){return[]}}
function remember(addr,symb){
  try{
    const a=String(addr).toLowerCase();
    const list=[{a,s:String(symb||'').slice(0,12),t:Date.now()}].concat(recent().filter(x=>x.a!==a)).slice(0,5);
    localStorage.setItem(RECENT_KEY,JSON.stringify(list));
  }catch(e){}
  drawTry();
}
function drawTry(){
  const row=$('sc-try');if(!row)return;
  row.textContent='';
  const add=(label,addr,cls)=>{const b=document.createElement('button');b.type='button';b.textContent=label;b.title=addr;if(cls)b.className=cls;
    b.addEventListener('click',()=>{$('sc-in').value=addr;showClear();scan(addr)});row.appendChild(b)};
  const l=document.createElement('span');l.className='sc-try-l';l.textContent='Try one:';row.appendChild(l);
  for(const [s,a] of TRY)add(s,a);
  const rec=recent().filter(x=>!TRY.some(([,a])=>a===x.a));
  if(rec.length){const l2=document.createElement('span');l2.className='sc-try-l';l2.textContent='· you scanned:';row.appendChild(l2);
